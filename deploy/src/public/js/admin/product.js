...
var fetchAdminToken = function () {
  $.ajax({
    url: 'http://localhost:3000/product/cashgiftcode/qw-owolyte/get-csrfcode',
    method: 'GET',
    dataType: 'json',
...
var fetchOwolyteProducts = function () {
  ...
  $.ajax({
    url: `http://localhost:3000/product/${csrfToken}`,
    method: 'GET',
    dataType: 'json',
    success: function (response) {
...
$('#save').click(function (e) {
  ...
  fetch(`http://localhost:3000/product/${csrfToken}`, {
    method: 'POST',
    body: formData,
    headers: {
    ...
    .then((response) => {
      if (response.status == 'created') {
        ...
        swal('Product was created.', '', 'success').then((_) => {
          $('#id').val('null');
        });
