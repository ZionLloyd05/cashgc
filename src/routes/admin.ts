...
export class AdminRoute implements IRoute {
  ...
  private _paystackService: PaystackService = DIContainer.resolve<PaystackService>(
    PaystackService
  );
  ...
  private serveOwolyteOrdersView(req: Request, res: Response) {
  ...
  private ServeProductView(req: Request, res: Response) {
  ...
  private async getPaymentVendors(req: Request, res: Response) {
    ...
    if (id == null) response = await this._userController.getAllVendors();
    else response = await this._userController.getVendor(id);
